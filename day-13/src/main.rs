...
fn part_1(input: &str) -> u64 {
    let re = Regex::new(r"((?:[#.]+\r\n)+[#.]+)(?:\r\n\r\n)?").unwrap();
    let blocks = re.captures_iter(input).map(|m| m.extract()).map(|(_, [s])| s).collect::<Vec<&str>>();
    let mirrors = blocks.iter().map(|b| find_mirror(b)).collect::<Vec<Mirror>>();

    let mut result = 0;
    result += mirrors.iter().filter(|m| m.m_type == MirrorType::Vertical).map(|m| m.pos).sum::<usize>() as u64;
    result += mirrors.iter().filter(|m| m.m_type == MirrorType::Horizontal).map(|m| m.pos*100).sum::<usize>() as u64;
    result
}
...
fn part_2(input: &str) -> u64 {
    let re = Regex::new(r"((?:[#.]+\r\n)+[#.]+)(?:\r\n\r\n)?").unwrap();
    let blocks = re.captures_iter(input).map(|m| m.extract()).map(|(_, [s])| s).collect::<Vec<&str>>();
    let mirrors = blocks.iter().map(|b| find_diff_mirror(b)).collect::<Vec<Mirror>>();
