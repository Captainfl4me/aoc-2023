...
fn part_1(input: &str) -> u64 {
    let mut map: Vec<Vec<Tile>> = input.lines().map(|l| {
        l.chars().map(|c| Tile::new(TileType::new(c))).collect()
    }).collect();
    ...
    loop {
        ...
        for beam in beams {
            if map[beam.y as usize][beam.x as usize].already_been_there(&beam) { continue; }
            let mut ap_tfr = map[beam.y as usize][beam.x as usize].apply_transform(&beam);
            ap_tfr.iter_mut().for_each(|b| b.apply_dir());            
            ap_tfr.iter().filter(|b| b.x >= 0 && b.y >= 0 && b.x < map[0].len() as i32 && b.y < map.len() as i32).for_each(|b| new_beams.push(*b));
        }
        beams = new_beams;
    }
...
fn part_2(input: &str) -> u64 {
    ...
    for start_dir in vec![BeamDirection::Up, BeamDirection::Down, BeamDirection::Left, BeamDirection::Right] {
        for start_x in 0..input.lines().next().unwrap().len() {
            for start_y in 0..input.lines().count() {
                if (start_x != 0 && start_x != input.lines().next().unwrap().len() - 1) && (start_y != 0 && start_y != input.lines().count() - 1) {
                    continue;
                }
                let mut map: Vec<Vec<Tile>> = input.lines().map(|l| {
                    l.chars().map(|c| Tile::new(TileType::new(c))).collect()
        ...
        match self.tile_type {
            ...
            TileType::Horizontal => {
                match beam.direction {
                    BeamDirection::Up | BeamDirection::Down => vec![Beam::new(beam.x, beam.y, BeamDirection::Left), Beam::new(beam.x, beam.y, BeamDirection::Right)],
                    _ => vec![*beam],
                }
            },
            TileType::Vertical => {
                match beam.direction {
                    BeamDirection::Left | BeamDirection::Right => vec![Beam::new(beam.x, beam.y, BeamDirection::Up), Beam::new(beam.x, beam.y, BeamDirection::Down)],
                    _ => vec![*beam],
                }
            },
            TileType::DashRight => {
